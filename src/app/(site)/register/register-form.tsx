...
import { registerMember, type AuthState } from '../auth-actions'
...
export function RegisterForm() {
  const [state, formAction] = useActionState<AuthState, FormData>(registerMember, {})
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div>
        <label className="mb-1.5 block text-sm font-semibold text-text">Username</label>
        <input name="username" required placeholder="AniFan" className={inputClass} />
      </div>
      <div>
      ...
      <div>
        ...
        <input name="password" type="password" autoComplete="new-password" required placeholder="At least 8 characters" className={inputClass} />
      </div>
      <SubmitButton />
